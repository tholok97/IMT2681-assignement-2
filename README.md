...
* **Heroku**. The code is written with Heroku in mind, so the easiest way to run it is using Heroku commands (see step 4). It's possible to use the code without heroku, but you'll have to supply the environment-variables defined in the .env file in some other way.
